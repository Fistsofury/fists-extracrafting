...
function CraftItem(source, item)
    ...
    exports.oxmysql:fetch('SELECT xp FROM crafting_xp WHERE charidentifier = ? AND category = ?', {identifier, recipe.category}, function(result)
        if #result == 0 then
            exports.oxmysql:execute('INSERT INTO crafting_xp (charidentifier, category, xp) VALUES (?, ?, 0)', {identifier, recipe.category})
        else
        ...
        exports.oxmysql:execute('UPDATE crafting_xp SET xp = xp + ? WHERE charidentifier = ? AND category = ?', {recipe.xpReward, identifier, recipe.category})
...
AddEventHandler("fists_crafting:craftItem1", function(item)
    print("r client event craft button clicked")
    local _source = source
